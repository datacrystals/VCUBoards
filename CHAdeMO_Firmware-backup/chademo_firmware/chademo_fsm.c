...
static void transition_to(chademo_context_t *ctx, chademo_state_t new_state)
{
    ...
    /* Reset CAN RX timeout whenever we enter a state that expects traffic.
     * Otherwise time spent in IDLE accumulates and causes an instant
     * timeout on plug insertion. */
    if (new_state >= CHADEMO_STATE_PLUG_DETECTED && new_state <= CHADEMO_STATE_CHARGING) {

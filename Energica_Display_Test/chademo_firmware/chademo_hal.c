...
void hal_gpio_init(void)
{
    ...
    gpio_put(PIN_OUT_DCP, 0);  /* DCP de-asserted */
#endif
}
...
static inline void cs_low(const can_hw_config_t *cfg)
{
    ...
    /* Tiny delay to satisfy MCP2515 t_CSC (CS setup to clock) = 50ns min */
    asm volatile("nop\nnop\nnop\nnop");
}

static inline void cs_high(const can_hw_config_t *cfg)
{
    /* Tiny delay to satisfy MCP2515 t_CSH (CS hold after clock) = 50ns min */
    asm volatile("nop\nnop\nnop\nnop");
...
bool hal_can_init(hal_can_channel_t ch)
{
     ...
     */
    hal_can_write_reg(ch, MCP2515_REG_RXB0CTRL, 0x60);  /* RXM1=1, RXM0=1 (receive any) */
    hal_can_write_reg(ch, MCP2515_REG_RXB1CTRL, 0x60);  /* Same for RXB1 */
...
/* ============================================================================
 * MCP2515 INITIALIZATION WITH SELECTABLE BITRATE
 * ============================================================================ */

bool hal_can_init_with_bitrate(hal_can_channel_t ch, uint16_t kbps)
{
    const can_hw_config_t *cfg = &can_cfg[ch];
     ...
     * All calculations assume 8 MHz crystal (most common on blue PCB modules).
     * TQ = 2*(BRP+1)/Fosc = 2*(BRP+1)/8MHz
     */
    uint8_t cnf1, cnf2, cnf3;

    switch (kbps) {
    case 1000:
        /* 1 Mbps @ 8MHz: 4 TQ total (aggressive but works on short buses)
         * TQ=250ns (BRP=0), 1(SYNC)+1(PROP)+1(PS1)+1(PS2)=4 TQ
         * Sample at 75% */
        cnf1 = 0x00; /* BRP=0, SJW=1, TQ=250ns */
        cnf2 = 0x90; /* BTLMODE=1, SAM=0, PHSEG1=1(PS1=2), PRSEG=0(PROP=1) */
        cnf3 = 0x00; /* PHSEG2=0(PS2=1) */
        break;

    case 500:
        /* 500 kbps @ 8MHz: 8 TQ total
         * TQ=250ns (BRP=0), 1(SYNC)+1(PROP)+4(PS1)+2(PS2)=8 TQ
         * Sample at 75% */
        cnf1 = 0x00; /* BRP=0, SJW=1, TQ=250ns */
        cnf2 = 0x98; /* BTLMODE=1, SAM=0, PHSEG1=3(PS1=4), PRSEG=0(PROP=1) */
        cnf3 = 0x01; /* PHSEG2=1(PS2=2) */
        break;

    case 250:
        /* 250 kbps @ 8MHz: 16 TQ total
         * TQ=250ns (BRP=0), 1(SYNC)+1(PROP)+8(PS1)+6(PS2)=16 TQ
         * Sample at 62.5% */
        cnf1 = 0x00; /* BRP=0, SJW=1, TQ=250ns */
        cnf2 = 0xB8; /* BTLMODE=1, SAM=0, PHSEG1=7(PS1=8), PRSEG=0(PROP=1) */
        cnf3 = 0x05; /* PHSEG2=5(PS2=6) */
        break;

    case 125:
        /* 125 kbps @ 8MHz: 16 TQ total, TQ=500ns (BRP=1)
         * 1(SYNC)+1(PROP)+8(PS1)+6(PS2)=16 TQ, 16*500ns=8us=125kbps */
        cnf1 = 0x01; /* BRP=1, SJW=1, TQ=500ns */
        cnf2 = 0xB8; /* BTLMODE=1, SAM=0, PHSEG1=7(PS1=8), PRSEG=0(PROP=1) */
        cnf3 = 0x05; /* PHSEG2=5(PS2=6) */
        break;
    ...
    hal_can_write_reg(ch, MCP2515_REG_CNF1, cnf1);
    hal_can_write_reg(ch, MCP2515_REG_CNF2, cnf2);
    hal_can_write_reg(ch, MCP2515_REG_CNF3, cnf3);
